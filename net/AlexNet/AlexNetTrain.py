...
class AlexTrain: 
    ...
    def getLenet(self):
        ...
        net = AlexNet.alexnet(True,num_classes=10).to(self.device)
    ...
    def train_net(self,net,loss_fuc,optimizer):
        ...
        adjust_lr_scheduler = lr_scheduler.StepLR(optimizer, step_size=50, gamma=0.1)#定义学习率衰减函数
        ...
        for epoch in range(self.EPOCH):
            ...
            for i,data in enumerate(train_loader):
                ...
                sum_loss += loss.item()
                #print(i)
                if iteration%2==0:#每100次Iteration，测试一次测试数据
                    lr = optimizer.param_groups[0]["lr"]#get current lr
                    #print(lr)
                    print('###iteration[:%d],[Epoch:%d],[Lr:%.08f] train loss: %.03f' % (iteration,epoch + 1, lr, sum_loss / 2))
                    #用网络测试验证数据
                    #保存模型
                    sum_loss = 0.0
    ...
    def load_model(self,path):
        net = AlexNet.alexnet(False,10).to(self.device)
